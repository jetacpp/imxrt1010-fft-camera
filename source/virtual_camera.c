...
static void USB_DeviceVideoPrepareVideoData(void)
{
    ...
    for (; g_UsbDeviceVideoVirtualCamera.imageBufferLength < maxPacketSize; g_UsbDeviceVideoVirtualCamera.imageBufferLength++)
    {

        uint8_t* frame = g_frames[g_cur_frame];
        uint32_t frame_len = g_frames_len[g_cur_frame];

        g_UsbDeviceVideoVirtualCamera.imageBuffer[g_UsbDeviceVideoVirtualCamera.imageBufferLength] = frame[g_UsbDeviceVideoVirtualCamera.imageIndex++];


        if ((0xFFU == frame[g_UsbDeviceVideoVirtualCamera.imageIndex - 2]) &&
            (0xD9U == frame[g_UsbDeviceVideoVirtualCamera.imageIndex - 1U]))        // end of frame
        {
            temp32dwFrameInterval
                USB_LONG_FROM_LITTLE_ENDIAN_DATA(g_UsbDeviceVideoVirtualCamera.commitStruct->dwFrameInterval);
            if (g_UsbDeviceVideoVirtualCamera.imageIndex >= frame_len)    
            {
                g_UsbDeviceVideoVirtualCamera.imageIndex = 0U;              // wrap around, go to beginning
            }
            if (g_UsbDeviceVideoVirtualCamera.currentTime < temp32dwFrameInterval)
            {
                g_UsbDeviceVideoVirtualCamera.waitForNewInterval = 1U;      // tell wait for device
            }
            else
            {
                // looks like null packet
                g_UsbDeviceVideoVirtualCamera.currentTime                = 0U;
                payloadHeader->headerInfoUnion.headerInfoBits.endOfFrame = 1U;
                g_UsbDeviceVideoVirtualCamera.stillImageTransmission     = 0U;
                g_UsbDeviceVideoVirtualCamera.currentFrameId ^= 1U;
                if (USB_DEVICE_VIDEO_STILL_IMAGE_TRIGGER_TRANSMIT_STILL_IMAGE == g_UsbDeviceVideoVirtualCamera.stillImageTriggerControl)
                {
            ...
            g_cur_frame = g_cur_frame ^ 0x01; // switch to next frame
            usb_echo("Switch to frame %d\r\n", g_cur_frame);
            break;
        }
    }
}
...
extern float do_fft(
    uint32_t sampleRate, uint32_t bitWidth, uint8_t *buffer, float *fftData, float *fftResult);
extern void gen_sine(uint8_t * in, uint32_t in_len, int fs);
extern void do_jpeg(float* signal, uint32_t signal_len, uint8_t* frame, uint32_t frame_len);
...
#endif
{
    ...
    if (emptyBlock > 0)
            ...
            if (emptyBlock < BUFFER_NUMBER)
            {
                ...
                //gen_sine(xfer.data, xfer.dataSize, 250);
                /* Compute the frequency of the data using FFT */
                maxFreq = do_fft(DEMO_AUDIO_SAMPLE_RATE, DEMO_AUDIO_BIT_WIDTH, xfer.data, ffData, ffResult);
                // {
                // 	static float prev;
                // 	if (prev != maxFreq && maxFreq < 15000) {
                // 		usb_echo("%d\r\n", (int)(maxFreq));
                // 		prev = maxFreq;
                // 	}
                // }
